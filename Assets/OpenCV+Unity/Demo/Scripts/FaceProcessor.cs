namespace OpenCvSharp.Demo
{
        ...
        where T: UnityEngine.Texture
    {
        ...
        public Vector2 gaze; // (-1 to 1 range)
        public Vector2 calibratedGaze;    // corrected gaze
        Vector2 calibrationOffset = Vector2.zero;
                ...
                for (int i = 0; i < rawFaces.Length; ++i)
                {
                    ...
                    using (Mat grayFace = new Mat(gray, faceRect))
                    {
                        ...
                        if (null != cascadeEyes)
                        {
                            ...
                            {
                                OpenCvSharp.Rect eye = eyes[e];
                                Point eyeCenter = new Point(
                                    faceRect.X + eye.X + eye.Width / 2,
                                    faceRect.Y + eye.Y + eye.Height / 2
                                    );

                                //Cv2.Circle(frameMat, eyeCenter, 5, new Scalar(0, 255, 0), 2);

                                // Approximate gaze direction
                                float normX = ((float)(eyeCenter.X - (faceRect.X + faceRect.Width / 2))) / faceRect.Width;
                                float normY = ((float)(eyeCenter.Y - (faceRect.Y + faceRect.Height / 2))) / faceRect.Height;

                                gaze = new Vector2(normX, -normY);

                                if (calibrated)
                                    calibratedGaze = gaze - calibrationOffset;
                                else
                                    calibratedGaze = gaze;
                            }
                        }
